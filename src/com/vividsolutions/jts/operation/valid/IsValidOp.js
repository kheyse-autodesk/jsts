function IsValidOp(parentGeometry) {
...
var ConnectedInteriorTester = require('com/vividsolutions/jts/operation/valid/ConnectedInteriorTester');
...
var MCPointInRing = require('com/vividsolutions/jts/algorithm/MCPointInRing');
...
var ConsistentAreaTester = require('com/vividsolutions/jts/operation/valid/ConsistentAreaTester');
var GeometryCollection = require('com/vividsolutions/jts/geom/GeometryCollection');
var IndexedNestedRingTester = require('com/vividsolutions/jts/operation/valid/IndexedNestedRingTester');
var RobustLineIntersector = require('com/vividsolutions/jts/algorithm/RobustLineIntersector');
var TopologyValidationError = require('com/vividsolutions/jts/operation/valid/TopologyValidationError');
var Assert = require('com/vividsolutions/jts/util/Assert');
IsValidOp.prototype.checkInvalidCoordinates = function (...args) {
	switch (args.length) {
		case 1:
...
IsValidOp.prototype.checkHolesNotNested = function (p, graph) {
...
IsValidOp.prototype.checkConsistentArea = function (graph) {
...
IsValidOp.prototype.isValid = function () {
	this.checkValid(this.parentGeometry);
	return this.validErr === null;
};
IsValidOp.prototype.checkShellInsideHole = function (shell, hole, graph) {
...
IsValidOp.prototype.checkNoSelfIntersectingRings = function (graph) {
...
IsValidOp.prototype.checkConnectedInteriors = function (graph) {
...
IsValidOp.prototype.checkNoSelfIntersectingRing = function (eiList) {
...
IsValidOp.prototype.checkHolesInShell = function (p, graph) {
...
IsValidOp.prototype.checkTooFewPoints = function (graph) {
	if (graph.hasTooFewPoints()) {
		this.validErr = new TopologyValidationError(TopologyValidationError.TOO_FEW_POINTS, graph.getInvalidPoint());
		return null;
	}
};
IsValidOp.prototype.getValidationError = function () {
	this.checkValid(this.parentGeometry);
	return this.validErr;
};
IsValidOp.prototype.checkValid = function (...args) {
	switch (args.length) {
		case 1:
...
IsValidOp.prototype.setSelfTouchingRingFormingHoleValid = function (isValid) {
	this.isSelfTouchingRingFormingHoleValid = isValid;
};
IsValidOp.prototype.checkShellNotNested = function (shell, p, graph) {
	var shellPts = shell.getCoordinates();
...
IsValidOp.prototype.checkClosedRings = function (poly) {
...
IsValidOp.prototype.checkClosedRing = function (ring) {
	if (!ring.isClosed()) {
		var pt = null;
...
IsValidOp.prototype.checkShellsNotNested = function (mp, graph) {
...
IsValidOp.isValid = function (...args) {
	switch (args.length) {
		case 1:
			if (args[0] instanceof Geometry) {
				return ((...args) => {
